    ...
    <div class="search_input">
      <div class="search_input_wrapper">
        ...
        <input type="text"
               v-model="kw"
               @blur="search()"
               placeholder="请输入你要搜索的影片的关键字">
      </div>
    </div>
    <div class="search_result">
      <h3>电影/电视剧/综艺</h3>
      <ul>
        <li v-for="movie in result"
            :key="movie.id">
          <div class="pic_show"><img :src="movie.img|updateWH('64.90')"></div>
          <div class="info">
...
export default {
  ...
  methods: {
    search () {
      this.$axios.get("/api/searchList?cityId=10&kw=" + this.kw).then((res) => {
        console.log(res)
        this.result = res.data.data.movies.list;
      })
